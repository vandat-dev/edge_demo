...
from frame_pb2 import FrameChunk
from frame_pb2_grpc import FrameStreamerStub
...
def generate_chunks(frames: np.ndarray) -> Iterable[FrameChunk]:
    for idx, frame in enumerate(frames):
        data = frame_to_png_bytes(frame)
        logger.info("Chuẩn bị gửi frame #%s", idx)
        yield FrameChunk(index=idx, data=data)
...
def send_frames(frames: np.ndarray, target: str):
    ...
    with grpc.insecure_channel(target) as channel:
        stub = FrameStreamerStub(channel)
        ack = stub.SendFrames(generate_chunks(frames))
